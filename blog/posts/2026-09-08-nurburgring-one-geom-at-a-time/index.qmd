...
around the clipped roads. Keep the attribution inside the map, below a compact
scale bar, so the exported image has no caption strip or outer margins.
...
map_credit <- function(bounds = map_bbox, colour = "#627064") {
  annotate(
    "text",
    # Inset the text 1.5% from the right and bottom edges of the projected frame.
    x = bounds[["xmax"]] - 0.015 * (bounds[["xmax"]] - bounds[["xmin"]]),
    y = bounds[["ymin"]] + 0.015 * (bounds[["ymax"]] - bounds[["ymin"]]),
...
# Keep the distance accurate but make its label and bar less prominent.
light_scale_bar <- ggspatial::annotation_scale(
  ...
  line_width = 0.4,
  # Leave room for the credit below the scale, inside the map.
  pad_y = grid::unit(0.60, "cm")
)
...
The map fills the image, with the credit and scale inside it, ready to use as a wallpaper.
