...
description: "Simulating autoregressive processes and watching them evolve with highcharter."
date: 2021-06-18
date-modified: 2026-08-13
categories: [minipost, stats, highcharter, time-series, data-visualization]
...
install_missing_packages(c(
  "highcharter", "stringr", "purrr", "htmltools"
))
```
...
```{r}
#| label: the-model-prepare-ar-model
...
```{r}
#| label: interactivity-prepare-ar
library(highcharter)
library(stringr)
...
```{r}
#| label: interactivity-prepare-load-fn
load_fn <- "function () {{
...
```{r}
#| label: improving-it-prepare-tick-post-fn
...
```{r}
#| label: improving-it-prepare-formula
...
```{r}
#| label: improving-it-prepare-formula-02
formula <- purrr::map2(ar, seq_along(ar), function(par, t){
...
```{r}
#| label: improving-it-prepare-rm-poinst-fn
...
```{r}
#| code-fold: true
#| label: extending-it-define-sim-ar-hc
library(highcharter)
library(stringr)
...
```{r}
#| label: extending-it-sim-ar-hc-02
sim_ar_hc(ar = c(.8, .1))
```

## References

- Highcharts. [Spline updating each second](https://jsfiddle.net/gh/get/library/pure/highcharts/highcharts/tree/master/samples/highcharts/demo/dynamic-update).
- Black Label. [Dynamic series example](https://jsfiddle.net/BlackLabel/evypfr1L/).
- JSFiddle. [Highcharts series update example](https://jsfiddle.net/upt4cbqj/).
- Java2s. [Remove the first data item from a Highcharts series](https://www.java2s.com/Tutorials/highcharts/Example/Series_Data/Remove_first_data_item_from_series.htm).
