...
date: 2020-06-24
description: "Combining a streamgraph and stacked columns to reveal two views of the same data."
...
install_missing_packages(c(
  "tidyverse", "lubridate", "highcharter", "shades", "readr",
  "RcppRoll", "janitor", "scales", "knitr"
))
```
...
# data <- data %>% 
#   filter(date <= ymd(20220701), wday(date) == 2)
# glimpse(data)
...
```{r}
#| label: data-rcpproll-roll-meanr
...
```{r}
#| label: the-streamgraph-prepare-ctry-to-separate
...
```{r}
#| label: the-streamgraph-prepare-data
data <- data %>% 
  group_by(continent) %>% 
  mutate(
    grp = case_when(
...
```{r}
#| label: the-streamgraph-prepare-palette
...
```{r}
#| label: the-streamgraph-prepare-lvls
...
```{r}
#| label: the-streamgraph-prepare-dcont-grp
dcont_grp <- data %>% 
...
```{r}
#| label: the-streamgraph-hchart-02
hchart(
  dcont_grp,
  "streamgraph",
...
```{r}
#| label: the-stacked-column-chart-hchart
hchart(
...
- Create and add two yAxis using `hc_yAxis_multiples` and `create_axis`
...
```{r}
#| code-fold: true
#| label: the-final-chart-prepare-hcft
hcft <- highchart() %>% 
  hc_add_series(
    dcont_grp,
    "streamgraph",
  ...
  hc_tooltip(
    ...
    style = list(fontSize = "0.7em", zIndex = 9999)
    ) %>%
  hc_legend(
    verticalAlign = "top",
    align = "left",
...
```{r}
#| label: the-final-chart-analysis
#| column: page
hcft %>% 
  hc_size(height = 600)
```
