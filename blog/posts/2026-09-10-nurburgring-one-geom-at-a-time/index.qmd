...
The blog preview uses a tighter frame and a simple cartographic casing: each
road is drawn once as a slightly wider dark border and again with its main colour.
This keeps tight curves and crossings readable at thumbnail size.
...
preview_roads <- roads_surroundings |>
  mutate(
    ...
    preview_border_colour = if_else(road_group == "Racetracks", asphalt_dark, "#526A76"),
    preview_border_width = preview_width + if_else(road_group == "Racetracks", 0.40, 0.08)
  )

preview_map <- ggplot(preview_roads) +
  # Draw a darker, wider casing first so tight curves and crossings stay distinct.
  geom_sf(
    aes(colour = preview_border_colour, linewidth = preview_border_width),
    lineend = "round"
  ) +
  geom_sf(
    aes(colour = preview_colour, linewidth = preview_width),
    lineend = "round"
  ) +
  scale_colour_identity() +
  scale_linewidth_identity() +
...
# Export this treatment as another full-size map version.
save_map(preview_map, "roads-preview")

# Keep the dedicated thumbnail filename used by the blog card.
ggsave(file.path("images", "preview.png"), preview_map, width = 8, height = 8 / screen_ratio, units = "in", dpi = 150, bg = forest_green)
```
