...
<script>document.body.style.setProperty("--blog-background", "#174A70")</script>
...
many_models_blue <- set_plot_accent_color("#174A70")
...
# Run this optional chunk to refresh the preview; the screenshot requires Chrome.
# Reuse the chart, lightening its continent colours for contrast in the thumbnail.
hc_preview <- hc |>
  hc_chart(
    backgroundColor = many_models_blue,
    plotBackgroundColor = many_models_blue
  ) |>
  hc_title(text = NULL) |>
  ...
  hc_colors(grDevices::adjustcolor(
    getOption("highcharter.theme")$colors,
    offset = c(0.2, 0.2, 0.2, 0)
  )) |>
  hc_plotOptions(series = list(
    animation = FALSE,
    opacity = 0.9,
    marker = list(lineColor = "#F7FBFF", lineWidth = 0.6)
  ))

post_dir <- "blog/posts/2026-07-20-many-models-many-years-later"
widget_file <- tempfile("many-models-chart-", fileext = ".html")
image_file <- file.path(post_dir, "blog/posts/2026-07-20-many-models-many-years-later/preview.png")

htmlwidgets::saveWidget(
  hc_size(hc_preview, height = 440),
  widget_file,
  selfcontained = FALSE,
  background = many_models_blue
)
