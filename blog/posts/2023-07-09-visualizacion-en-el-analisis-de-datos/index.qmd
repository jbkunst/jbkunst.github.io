...
lang: es
description: "Explorando patrones, relaciones y estructuras en datos del Metro de Santiago."
date: 2023-07-09
...
<script>document.body.style.setProperty("--blog-background", "#00838F")</script>

```{r setup, include=FALSE}
source("blog/_R/post_setup.R")

metro_teal <- set_plot_accent_color("#00838F")

install_missing_packages(c(
  "tidyverse", "scales", "corrr", "patchwork", "widyr", "igraph",
  "ggnetwork", "ggforce", "chilemapas", "uwot", "ggdendro", "ape",
  "data.table", "fs", "readr", "corrplot", "ggrepel", "R.utils"
))
```
...
```{r metrodata}
ruta_datos <- "blog/posts/2023-07-09-visualizacion-en-el-analisis-de-datos/data"

data <- data.table::fread(
  fs::path(ruta_datos, "2015.04_Subidas_paradero_mediahora_web.csv.gz")
  )
data <- as_tibble(data)
...
```{r}
#| label: complete-station-time-grid
data <- complete(
  data,
  paraderosubida,
  mediahora, 
...
```{r}
#| label: plot-plaza-maipu-profile
#| column: page
dplazamaipu <- data |>
...
```{r}
#| label: plot-plaza-maipu-horizontal
#| column: page
ggplot(dplazamaipu) +
...
```{r}
#| label: plot-plaza-maipu-time-series
#| column: page
ggplot(dplazamaipu) +
...
```{r}
#| label: compare-station-patterns
#| column: page
d1 <- data |> 
...
```{r}
#| label: simulate-correlation-example
#| column: page
set.seed(123)
...
```{r}
#| label: annotate-station-correlation
#| column: page
lab_dates <- d1 |> 
...
```{r}
#| label: compare-second-station-pair
#| column: page
d1 <- data |> 
...
```{r}
#| label: calculate-pairwise-correlations
dcor <- data |>
  widyr::pairwise_cor(
    paraderosubida,
    mediahora,
...
```{r}
#| label: order-correlation-matrix
#| column: page
M <- data |> 
  spread(paraderosubida, subidas_laboral_promedio) |> 
  select(-1) |> 
...
```{r}
#| label: select-network-correlations
ncors <- 250
...
```{r}
#| code-fold: true
#| label: detect-network-communities
#| column: page
...
```{r}
#| label: prepare-station-feature-matrix
nhead <- 8
...
```{r}
#| label: evaluate-kmeans-clusters
#| column: page
...
```{r}
#| label: build-hierarchical-clustering
#| column: page
library(ggdendro)
...
```{r}
#| label: compare-cluster-profiles
#| column: page
dataf <- data |> 
...
```{r}
#| code-fold: true
#| label: load-transit-network-data
...
```{r}
#| label: summarize-analysis-plots
#| column: page
#| code-fold: true
#| code-summary: "Mostrar código del resumen"
...
pdend_summary <- remove_geom(pdend, "GeomText")

list(p0, p1, p2, pcors, pnet, pumapkm, pkm,
     pdend_summary, pphylo, pclus, pmetro, pmetro2) |>
  map(function(p){
...
::: {.callout-tip}
## ¿Cuándo necesita ejes una visualización?

Quitar títulos, ejes o leyendas deja más espacio para el panel y puede hacer que
el gráfico parezca más grande, pero esa limpieza solo ayuda cuando esos elementos
no son necesarios para interpretar la figura. En series, dispersiones y barras,
los ejes suelen ser indispensables para leer magnitudes y unidades; en matrices
y mapas de calor, además identifican las categorías comparadas.

En mapas temáticos, las coordenadas de longitud y latitud normalmente aportan
poco si el objetivo es reconocer patrones espaciales, aunque una escala gráfica
sí es útil cuando importan las distancias. En redes y proyecciones como UMAP, las
coordenadas suelen ser relativas y sus valores numéricos pueden omitirse. En un
PCA, en cambio, los componentes y la varianza explicada sí tienen interpretación.
Un título dentro del gráfico también puede omitirse cuando el encabezado y el
texto del artículo ya entregan el contexto necesario.
:::
