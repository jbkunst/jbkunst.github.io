...
# Internal site theming; keep this value in sync with forest_green in Map area.
invisible(set_plot_accent_color("#47714D"))
...
In this post we build the map progressively: first the surrounding road network,
...
::: {.column-body}
![](images/preview.png){width=100% fig-alt="A stylised map of Nürburgring with the racetrack highlighted against the surrounding road network."}
:::

## Map area

Set the screen resolution, map width and palette here. Height follows the screen ratio.
...
# Map palette: forest green for the circuit and cool asphalt greys for the canvas.
forest_green <- "#47714D"
...
osm_file <- file.path("blog/posts/2026-09-10-nurburgring-one-geom-at-a-time/data", paste0("osm-", area_id, ".rds"))
...
images_dir <- "blog/posts/2026-09-10-nurburgring-one-geom-at-a-time/images"
...
The blog preview uses a tighter frame. Nearby roads stay as a single quiet layer,
while the circuit gets a two-stroke cartographic casing: a dark outer line and a
narrower pale line. This keeps the GP complex legible without giving every road
a heavy border.
...
# Keep context roads thin and uncased; reserve the stronger treatment for the circuit.
preview_context <- roads_context |>
  mutate(preview_width = pmax(line_width, 0.10))

preview_map <- ggplot() +
  geom_sf(
    data = preview_context,
    aes(linewidth = preview_width),
    colour = "#718894",
    lineend = "round"
  ) +
  # Draw the circuit twice: a dark casing first, then a narrower pale stroke.
  geom_sf(
    data = raceways,
    colour = asphalt_dark,
    linewidth = 1.35,
...
# Use the same base name for the blog card PNG and its scalable SVG counterpart.
save_map(preview_map, "preview")
```

SVG downloads:
[Uniform roads](images/roads-uniform.svg){download=""} ·
[Road hierarchy](images/roads-hierarchy.svg){download=""} ·
[Surroundings](images/roads-surroundings.svg){download=""} ·
[Light version](images/roads-light.svg){download=""} ·
[Dark version](images/roads-dark.svg){download=""} ·
[Circuit sections](images/circuit-sections.svg){download=""} ·
[Sprintstrecke](images/sprintstrecke.svg){download=""} ·
[Preview](images/preview.svg){download=""}
