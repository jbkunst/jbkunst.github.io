...
date: "2020-05-12"
description: "Diez ejercicios de visualización con R, ggplot2 y highcharter."
date-modified: 2026-08-13
categories: [spanish, highcharter, data-visualization, ggplot2]
...
install_missing_packages(c(
  "datos", "highcharter", "ggplot2", "dplyr", "tidyr", "forcats",
  "scales", "ggridges", "lubridate", "htmltools", "paletteer",
  "reshape2", "knitr", "ggplot2movies", "scico"
))
```
...
:::{.column-body}
![30dias](images/listado30graficosaldia.jpg)
:::
...
-   Haré la mayoría de gráficos con el paquete [{highcharter}](http://jkunst.com/highcharter), intentando ser lo más claro posible con el fin de que sirva también como introducción, casos de usos y ejemplos para el paquete. Partiremos de menos a más: desde las funciones básicas, complementando luego con mejores práctivas y agregando funcionalidades desde le mundo Javascript y HTML.
...
## Día 1: barras/columnas {#día-1-barrascolumnas}
...
```{r}
#| label: dia-1-barras-columnas-prepare-conteo-clases
...
```{r}
#| label: dia-1-barras-columnas-hchart
...
```{r}
#| label: dia-1-barras-columnas-prepare-conteo-clases-02
...
```{r}
#| label: dia-2-lineas-prepare-continentes
continentes <- paises %>% 
...
```{r}
#| label: dia-2-lineas-prepare-hc02
...
```{r}
#| label: dia-3-puntos-burbujas-prepare-paises-2007
...
```{r}
#| label: dia-3-puntos-burbujas-hchart
...
```{r}
#| label: dia-3-puntos-burbujas-prepare-texto-explicativo
...
```{r}
#| label: dia-4-graficos-con-facetas-prepare-flores-auxiliar
...
```{r}
#| label: dia-4-graficos-con-facetas-prepare-hc04
hc04 <- hchart(
  flores, 
  "scatter",
...
**En lo personal** el layout no me agrada mucho pues no lo considero algo que aporte. De todas formas existe mucha documentación para realizar este tipo de gráficos con {ggplot2} con ayuda de la librería {ggraph} cuya documentación está [acá](https://ggraph.data-imaginist.com/index.html).
...
```{r}
#| label: dia-6-donut-prepare-diamantes-cortes
library(scales)
...
El gráfico de tipo *ridgeline* viene a solucionar la superposición de muchas densidades (o histogramas) haciendo distintos ejes x dependiendo de una variable. Trabajarlo con {ggplot2} es muy sencillo dada la extensión {ggridges} por [Claus Wilke](https://twitter.com/clauswilke).

```{r}
#| label: dia-7-ridgeline-prepare-clima
...
```{r}
#| label: dia-8-contorno-prepare-volcano3d
...
```{r}
#| label: dia-8-contorno-prepare-dd08
...
```{r}
#| label: dia-8-contorno-prepare-hc08
hc08 <- hchart(
  dd08,
  "line",
...
```{r}
#| label: dia-9-areas-apiladas-data
...
**Medio**. Crearemos un data frame de eventos temporales, los cuales pasando a listas en un formato reconocido por highchartsJS agregaremos al gráfico como `plotLine`s (existen también las `plotBand`s).

```{r}
#| label: dia-9-areas-apiladas-prepare-eventos
eventos <- tibble(
...
Como segundo ejercicio ilustrativo, podemos agregar más detalles al gráfico obtenido en en los [gráficos de columnas](#día-1-barrascolumnas).

```{r}
#| label: dia-9-areas-apiladas-prepare-conteo-clases-millas
...
```{r}
#| label: dia-10-paletas-de-colores-prepare-colores-ggplot2-default
library(scales)
...
```{r}
#| label: dia-10-paletas-de-colores-prepare-colores-viridis
...
```{r}
#| label: dia-10-paletas-de-colores-prepare-paleta-scico
...
```{r}
#| label: dia-10-paletas-de-colores-prepare-colores
...
```{r}
#| label: dia-10-paletas-de-colores-hchart
hchart(
  flores, "scatter", 
  hcaes(Largo.Sepalo, Ancho.Sepalo, group = Especie),
...
```{r, echo=FALSE}
#| label: mapeo-mapping-knitr-include-graphics
#| column: body
knitr::include_graphics("blog/posts/2020-05-12-30diasdegraficos-parte-1/images/4_bubble-chart_mapping-vars.png")
```
