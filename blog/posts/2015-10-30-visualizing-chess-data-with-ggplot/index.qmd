...
description: A collection of chess visualizations created entirely with ggplot2.
date: 2015-10-30
...
install_missing_packages(c(
  "tidyverse", "furrr", "igraph", "doParallel", "plyr", "scales"
))

# rchess and ForceAtlas2 are installed from GitHub, not CRAN.
```
...
```{r load-chess-data-and-theme}
library(tidyverse)
library(ggplot2)
library(rchess)

chess_brown <- "#5D4037"
chess_board_colors <- c("#3E2723", chess_brown)
...
```{r prepare-all-game-moves, cache=TRUE}
chesswc <- chesswc %>%
...
f1_bishop_plot <- ggplot() +
  ...
  scale_fill_manual(values = chess_board_colors) +
  ggtitle("f1 Bishop") +
  coord_equal()
...
```{r}
#| label: create-f1-bishop-preview
#| include: false

# Reuse the bishop plot, keeping its square board and removing the thumbnail title.
f1_bishop_preview <- f1_bishop_plot +
  labs(title = NULL) +
  theme(
    plot.background = element_rect(fill = chess_brown, colour = NA),
    panel.background = element_rect(fill = chess_brown, colour = NA),
    plot.margin = margin(0, 0, 0, 0)
  )

dir.create("blog/posts/2015-10-30-visualizing-chess-data-with-ggplot/images", showWarnings = FALSE)
ggsave(
  "blog/posts/2015-10-30-visualizing-chess-data-with-ggplot/images/preview.png",
  f1_bishop_preview,
  width = 12,
  height = 6.75,
  dpi = 180,
...
ggplot(dfsurvrates) +
  ...
  scale_fill_gradient(low = chess_brown, high = "#EFEBE9") +
...
```{r plot-survival-rates}
#| column: page
ggplot(dfsurvrates) +
  geom_tile(data = dfsurvrates %>% filter(!is.na(surv_rate)),
            aes(x, y, fill = 100*surv_rate)) +
  scale_fill_gradient(NULL, low = chess_brown, high = "#EFEBE9") +
  geom_text(data = dfsurvrates %>% filter(!is.na(surv_rate)),
            aes(x, y, label = unicode), size = 11, color = "gray20",
            alpha = 0.7, family = "Segoe UI Symbol") +
  scale_x_continuous(breaks = 1:8, labels = letters[1:8]) +
  scale_y_continuous(breaks = 1:8, labels = 1:8)  +
  geom_segment(data = dfboard2, aes(x, y, xend = xend, yend = yend), color = "gray70") +
  geom_segment(data = dfboard2, aes(y, x, xend = yend, yend = xend), color = "gray70") +
...
```{r prepare-first-move-distributions}
#| column: page
...
ggplot(dfmoves_first_mvm) +
  geom_density(aes(number_move_2), fill = chess_brown, alpha = 0.8, color = NA) +
