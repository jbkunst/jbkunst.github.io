...
map_credit <- function(bounds = map_bbox, colour = "#627064") {
  annotate(
    "text",
...
road_style <- tibble::tribble(
  ...
  "Other",                         0.06, "#D3D8D5",    "#3B413E",
  "Non-motorised paths",           0.06, "#CDD3CF",    "#3F4742",
  "Farm and forest tracks",        0.08, "#C1CAC4",    "#48534B",
  "Service roads",                 0.10, "#B8C2BB",    "#515E55",
  "Local roads",                   0.14, "#A5B2A9",    "#606F65",
  "Regional roads",                0.23, "#8C9E91",    "#798D80",
  "Main roads",                    0.30, "#748B7B",    "#90A697",
  "Motorways",                     0.38, "#597362",    "#A6BAAC",
...
`ggforce::geom_mark_hull()` wrap the full highlighted section. For this annotated
view, remove the surrounding roads and waterways so the raceway geometry stays
in focus. The short annotations below paraphrase the [official Nürburgring Nordschleife guide](https://mobile.nuerburgring.de/info/nuerburgring/race-tracks/nordschleife?locale=en).

```{r}
#| label: draw-featured-raceway-sections
#| column: page
#| fig-cap: "Nine Nordschleife sections highlighted and annotated from their complete track geometry."
#| fig-alt: "The Nürburgring raceway on a dark green background, with nine famous Nordschleife sections highlighted in pale green and enclosed by subtle hull annotations."
...
raceway_aux <- raceways |>
  filter(raceway_name %in% section_names)

section_descriptions <- tibble::tribble(
  ~section,           ~description,
  "Flugplatz",        "Named for the former glider airfield beside the track.",
  "Schwedenkreuz",    "Named for the old stone cross beside this part of the circuit.",
  "Fuchsröhre",       "Named after a fox that occupied a construction pipe here in the 1920s.",
  "Adenauer Forst",   "The forest area belonging to the municipality of Adenau.",
  "Bergwerk",         "Named for the former lead and silver mine that operated here until around 1900.",
  "Karussell",        "The banked Caracciola-Karussell was concreted in 1936 and named for Rudolf Caracciola.",
  "Brünnchen",        "A former spring area and one of the Nordschleife's best-known spectator points.",
  "Pflanzgarten",     "Named for former gardens and cultivated fields of the Counts of Nürburg.",
  "Döttinger Höhe",   "Named after the nearby village of Döttingen."
)

track_sections <- raceway_aux |>
  mutate(section = raceway_name) |>
  group_by(section) |>
  summarise() |>
  mutate(
    length_m = round(as.numeric(sf::st_length(geometry))),
    section_label = if_else(section == "Karussell", "Caracciola-Karussell", section)
  ) |>
  left_join(section_descriptions, by = "section") |>
  mutate(label = paste0(section_label, " · ", length_m, " m"))

# Densify the same section lines so each hull follows the complete track segment.
...
sections_track <- "#A6BAAC"
sections_highlight <- "#EAF1E8"
...
p_sections <- ggplot() +
  ...
  ggforce::geom_mark_hull(
    ...
    label.fill = sections_background,
    label.colour = "#F1F5F2",
...
`Nürburgring Sprintstrecke`. Plotting it alone makes its shape easier to read.

```{r}
#| label: draw-sprintstrecke
#| column: page
#| fig-cap: "Nürburgring Sprintstrecke isolated from the rest of the raceway geometry."
#| fig-alt: "The compact Nürburgring Sprintstrecke drawn alone in forest green."
...
sprint_bbox <- sf::st_bbox(sprintstrecke)
sprint_pad_x <- 0.10 * (sprint_bbox[["xmax"]] - sprint_bbox[["xmin"]])
sprint_pad_y <- 0.10 * (sprint_bbox[["ymax"]] - sprint_bbox[["ymin"]])
sprint_plot_bbox <- sprint_bbox
sprint_plot_bbox[c("xmin", "xmax")] <- sprint_bbox[c("xmin", "xmax")] + c(-sprint_pad_x, sprint_pad_x)
sprint_plot_bbox[c("ymin", "ymax")] <- sprint_bbox[c("ymin", "ymax")] + c(-sprint_pad_y, sprint_pad_y)
...
sprint_map <- ggplot() +
  ...
  map_credit(bounds = sprint_plot_bbox) +
  map_theme +
  coord_sf(
    crs = 25832,
    xlim = sprint_plot_bbox[c("xmin", "xmax")],
    ylim = sprint_plot_bbox[c("ymin", "ymax")],
    expand = FALSE,
    datum = NA
  )
...
The map fills the image, with only a small credit inside it, ready to use as a wallpaper.
