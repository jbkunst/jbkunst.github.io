...
description: Animating how Game of Thrones characters shift affiliations across seasons.
date: 2019-04-23
...
<script>document.body.style.cssText += "--blog-background:#171717;--blog-hover:#d8d8d8"</script>

```{r setup, include=FALSE}
source("blog/_R/post_setup.R")

install_missing_packages(c(
  "tidyverse", "janitor", "igraph", "gridExtra", "ggrepel",
  "gganimate", "gifski", "extrafont"
))
```
...
::: {.column-body}
![Inspiration by FlowingData](https://i2.wp.com/flowingdata.com/wp-content/uploads/2016/08/floaty-bubbles-featured.gif?fit=720%2C449&ssl=1){width=100% fig-align="center"}
:::
...
::: {.column-body}
![Shifting affiliations visualized as an alluvial diagram](https://cdn-images-1.medium.com/max/2560/1*gJxTRX_NmrKmHFxamJYvyg.png){width=100% fig-align="center"}
:::

In this case the data can be downloaded from this repository https://github.com/MattLunkes/GoT_Affiliations.

```{r load-game-of-thrones-data}
library(tidyverse)
...
```{r summarize-affiliation-changes}
ts <- data_long |> 
  distinct(season) |> 
  pull() |> 
  head(-1)
...
```{r compare-affiliation-layouts, echo=FALSE}
#| column: body
library(gridExtra)
...
```{r render-affiliation-animation, eval=FALSE}
animate(p, fps = 30, duration = 8*3, width = 1000, height = 800)
```


```{r save-affiliation-animation, include=FALSE, eval=FALSE}
anim_save(
  animation = p, fps = 30, duration = 8*3, width = 1000, height = 800, filename = "blog/posts/2019-04-23-got-animating-the-shifting-of-affiliations/images/got-shifting-gganimate.gif"
  )
```
...
::: {.column-body}
![Animated shifting affiliations](images/got-shifting-gganimate.gif){width=100% fig-align="center"}
:::
