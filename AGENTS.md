...
- Para un cambio en un post, modificar y, cuando corresponda, renderizar únicamente ese post.
- Nunca ejecutar un render completo del sitio por un cambio local. Un render completo solo está permitido cuando el cambio afecta deliberadamente infraestructura compartida, como los defaults de los posts, la configuración global de gráficos, `blog/_R/post_setup.R`, `_quarto.yml` o estilos compartidos, y verificar todo el sitio resulta necesario.
...
- Mantener encapsulados dentro de `blog/_R/post_setup.R` los colores y fuentes internos; el helper no debe dejar variables auxiliares `plot_*` en el entorno del documento. Cuando un geom especializado necesite la fuente o el fondo compartidos, obtenerlos desde `ggplot2::theme_get()` en vez de duplicar sus valores. `set_plot_accent_color()` es infraestructura interna del sitio: llamarlo dentro del chunk `setup` con `include: false` y no mostrar esa llamada como parte del código pedagógico. Si el color forma parte del ejemplo, declarar además el valor editable en un chunk visible del post. Los overrides temáticos deben permanecer locales al post.
- Para series temporales o perfiles ordenados, usar `geom_line()` sin `geom_point()` por defecto. Combinar líneas y puntos solo cuando los puntos individuales tengan un propósito analítico claro, por ejemplo destacar observaciones discretas, mediciones escasas o valores seleccionados.
...
- Dar continuidad visual entre la tarjeta y el artículo usando el color editorial de `--blog-background` como fondo del preview. Reutilizar en R el color local del post; la llamada a `set_plot_accent_color()` permanece en el `setup` oculto según la convención anterior.
