...
names stay visible as `Unnamed` rather than being dropped.
...
When there are eight or fewer distinct labels, facet them against the same grey
base so each panel shows where that named group sits within all raceway geometry.

```{r}
#| label: facet-raceway-names
#| column: page
#| fig-height: 7
#| fig-cap: "Named raceway groups highlighted against all raceway geometry."

if (nrow(raceway_summary) <= 8) {
  raceways <- raceways |>
    mutate(
      raceway_name = factor(raceway_name, levels = raceway_summary$raceway_name)
    )

  # A layer without the facet variable is repeated as the grey base in every panel.
  raceway_base <- raceways |>
    select(-raceway_name, -length_km)

  ggplot() +
    geom_sf(data = raceway_base, colour = "#BCC3BE", linewidth = 0.25) +
    geom_sf(data = raceways, colour = forest_green, linewidth = 0.75) +
    facet_wrap(vars(raceway_name), ncol = 2) +
    map_theme +
    map_coords +
