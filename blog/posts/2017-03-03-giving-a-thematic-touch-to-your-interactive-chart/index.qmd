...
description: Adding thematic context and visual character to interactive charts.
date: 2017-03-03
...
install_missing_packages(c(
  "tidyverse", "jsonlite", "highcharter", "rvest", "countrycode", "janitor"
))
```
...
![Iraq's bloody toll](http://cdn3.i-scmp.com/sites/default/files/styles/980w/public/2013/07/17/iraqdeaths.jpg){width=80% fig-align="center"}
...
dgames <- tables |>
  ...
  purrr::keep(
    ~ "year" %in% names(.x) &&
      any(c("host", "host_city") %in% names(.x)) &&
      any(c("nations", "countries") %in% names(.x))
  ) |>
  purrr::pluck(1) |>
  tibble::as_tibble()

if ("host_city" %in% names(dgames)) {
  dgames <- dplyr::rename(dgames, host = host_city)
}

if ("countries" %in% names(dgames)) {
  dgames <- dplyr::rename(dgames, nations = countries)
}
