...
date: 2020-05-22
description: "Diez nuevas visualizaciones y recursos interactivos con highcharter."
...
install_missing_packages(c(
  "datos", "highcharter", "ggplot2", "dplyr", "tidyr", "lubridate",
  "forecast", "readr", "ggdendro", "geojsonio", "scales", "igraph",
  "Matrix", "ggraph", "graphlayouts", "countrycode", "economiccomplexity",
  "purrr", "xml2", "rvest", "stringr", "forcats", "jsonlite",
  "aimsir17", "ggplot2movies"
))
# sinimr is installed separately with remotes::install_github("robsalasco/sinimr")
```
...
```{r}
#| label: introduccion-prepare-newlang-opts
...
```{r}
#| label: dia-11-mapas-de-calor-prepare-mtautos2
...
```{r}
#| label: dia-11-mapas-de-calor-hchart
hchart(cor(mtautos))
```
...
```{r}
#| label: dia-12-paleta-lollipop-prepare-mtautos2
...
```{r}
#| label: dia-12-paleta-lollipop-prepare-x
...
```{r}
#| label: dia-13-visualizar-datos-temporales-data
data(co2)
...
```{r}
#| label: dia-13-visualizar-datos-temporales-prepare-descomposicion
...
```{r}
#| label: dia-13-visualizar-datos-temporales-prepare-pronosticos
library(forecast)
...
```{r}
#| label: dia-14-treemaps-prepare-conteo-clases
...
```{r}
#| label: dia-14-treemaps-prepare-url-base
library(readr)
...
```{r pkmn_colores_tipo}
# The original post scraped these colors from a wiki that is no longer
# available. Keeping the palette here makes the example reproducible.
pkmn_colores_tipo <- tribble(
  ~type,      ~color,
  "normal",  "#A8A878",
  "fighting","#C03028",
  "flying",  "#A890F0",
...
```{r}
#| label: dia-16-graficos-de-waffle-hchart
hchart(
  diamantes_cortes_1000,
  "item", 
  hcaes(name = corte, y = n),
...
```{r, eval=FALSE}
#| label: dia-18-datos-espaciales-prepare-url-gs-geojson
...
```{r, eval=FALSE}
#| label: dia-18-datos-espaciales-prepare-varcode
# remotes::install_github("robsalasco/sinimr")
library(sinimr)
...
```{r, eval=FALSE}
#| label: dia-18-datos-espaciales-highchart
highchart(type = "map") %>%
  hc_add_series(
...
```{r, eval=FALSE}
#| label: dia-18-datos-espaciales-prepare-dcovid
...
```{r, eval=FALSE}
#| label: dia-18-datos-espaciales-prepare-hc18
