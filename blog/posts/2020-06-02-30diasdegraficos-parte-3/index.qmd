...
description: "Más experimentos con anotaciones, texto, mapas y visualizaciones jerárquicas."
date: 2020-06-02
...
install_missing_packages(c(
  "datos", "highcharter", "ggplot2", "readr", "dplyr", "tidyr",
  "stringr", "rvest", "tidytext", "treemap", "lubridate", "purrr",
  "stopwords", "rlang"
))
```
...
```{r}
#| label: dia-21-graficos-con-anotaciones-prepare-tourfrance
tourfrance <- read_csv("blog/posts/2020-06-02-30diasdegraficos-parte-3/data/tour_france_state_8.txt")
...
```{r}
#| code-fold: true
#| label: dia-21-graficos-con-anotaciones-prepare-df1
#| column: page
...
```{r}
#| label: dia-22-datos-textuales-read-html
library(stringr)
library(rvest)
...
```{r}
#| label: dia-22-datos-textuales-prepare-r4ds2
library(tidytext)
...
```{r}
#| label: dia-22-datos-textuales-prepare-r4ds2-top50
r4ds2_top50 <- r4ds2 %>%
  arrange(desc(tf_idf)) %>%
  mutate(palabra = factor(palabra, levels = rev(unique(palabra)))) %>% 
  group_by(capitulo) %>% 
...
```{r}
#| code-fold: true
#| label: dia-23-proyeccion-solar-sunburst-prepare-paises-2007
...
```{r}
#| label: dia-24-coropleta-ya-lo-hicimos-antes-prepare-aeropuertos
...
```{r}
#| label: dia-25-violin-prepare-p25
library(ggplot2)
