...
OpenStreetMap stores the Nordschleife as many individually named raceway ways.
To make that structure visible without showing every label, highlight seven
well-known sections from the [official Nürburgring Nordschleife guide](https://nuerburgring.de/info/nuerburgring/race-tracks/nordschleife).
The guide calls the banked turn Caracciola-Karussell; its OSM way is named
`Karussell`.

```{r}
#| label: draw-featured-raceway-sections
#| column: page
#| fig-cap: "Seven named Nordschleife sections highlighted within the raceway geometry."
#| fig-alt: "The Nürburgring raceway geometry in grey, with seven famous Nordschleife sections highlighted and circled in green."

featured_section_names <- c(
  "Flugplatz",
  "Schwedenkreuz",
  "Fuchsröhre",
  "Karussell",
...
featured_sections <- raceways |>
  filter(raceway_name %in% featured_section_names)

stopifnot(n_distinct(featured_sections$raceway_name) == length(featured_section_names))

featured_points <- featured_sections |>
  group_by(raceway_name) |>
  summarise() |>
  sf::st_point_on_surface()

featured_points_plot <- cbind(
  sf::st_drop_geometry(featured_points),
  as.data.frame(sf::st_coordinates(featured_points))
) |>
  mutate(
    section_label = case_match(
      raceway_name,
      "Karussell" ~ "Caracciola-Karussell",
      .default = raceway_name
    ),
    radius = 260
  )

raceway_bbox <- sf::st_bbox(raceways)
raceway_pad_x <- 0.05 * (raceway_bbox[["xmax"]] - raceway_bbox[["xmin"]])
raceway_pad_y <- 0.05 * (raceway_bbox[["ymax"]] - raceway_bbox[["ymin"]])
raceway_plot_bbox <- raceway_bbox
raceway_plot_bbox[c("xmin", "xmax")] <- raceway_bbox[c("xmin", "xmax")] + c(-raceway_pad_x, raceway_pad_x)
raceway_plot_bbox[c("ymin", "ymax")] <- raceway_bbox[c("ymin", "ymax")] + c(-raceway_pad_y, raceway_pad_y)

raceway_detail_map <- ggplot() +
  geom_sf(data = raceways, colour = "#BCC3BE", linewidth = 0.30, lineend = "round") +
  geom_sf(data = featured_sections, colour = forest_green, linewidth = 0.90, lineend = "round") +
  ggforce::geom_circle(
    data = featured_points_plot,
  ...
  geom_text(
    data = featured_points_plot,
    aes(x = X, y = Y, label = section_label),
    family = map_font,
    colour = "#46564B",
    size = 2.4,
    nudge_y = 330,
  ...
  map_credit(bounds = raceway_plot_bbox) +
  map_theme +
  coord_sf(
    crs = 25832,
    xlim = raceway_plot_bbox[c("xmin", "xmax")],
    ylim = raceway_plot_bbox[c("ymin", "ymax")],
    expand = FALSE,
    datum = NA
  )
