...
focus_type <- htmlwidgets::JS(
  "function () {
    var typeKey = this.options.custom.typeKey;
    this.chart.series.forEach(function (series) {
      var sameType = series.options.custom &&
        series.options.custom.typeKey === typeKey;
      var opacity = sameType ? 1 : 0.12;

      if (series.group) series.group.attr({ opacity: opacity });
      if (series.markerGroup) series.markerGroup.attr({ opacity: opacity });

      if (sameType) {
        if (series.group) series.group.toFront();
        if (series.markerGroup) series.markerGroup.toFront();
      }
    });
    ...
    window.setTimeout(function () {
      ...
      if (chart.tooltip && chart.tooltip.container) {
        chart.tooltip.container.style.zIndex = 9999;
      }
    }, 0);
  }"
)

reset_types <- htmlwidgets::JS(
  "function () {
    this.chart.series.forEach(function (series) {
      if (series.group) series.group.attr({ opacity: 1 });
      if (series.markerGroup) series.markerGroup.attr({ opacity: 1 });
    });
  }"
)
  ...
  purrr::map(function(type_key) {
    ...
    list(
      ...
      list(
        ...
        custom = list(typeKey = type_key),
        events = list(
          mouseOver = focus_type,
          mouseOut = reset_types
        ),
        zIndex = 2
      )
    )
